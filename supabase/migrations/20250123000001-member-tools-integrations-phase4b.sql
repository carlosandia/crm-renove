-- =====================================================
-- MIGRATION: FASE 4B - MEMBER TOOLS & INTEGRATIONS
-- Version: 4B.1.0
-- Date: 2025-01-23
...
VALUES ('4B.1.0', 'FASE 4B: Member Tools & Integrations - Tasks, Calendar, Email, WhatsApp, Dashboard', NOW());
...
-- Create permissive policies for development (to be refined in production)
DO $$
DECLARE
    table_name TEXT;
    tables TEXT[] := ARRAY[
        ...
        EXECUTE format('
            ...
            USING (
                tenant_id IN (
                    ...
                    UNION ALL
                    SELECT id FROM companies WHERE id = tenant_id AND auth.uid() IS NULL
                )
            )
...
BEGIN
    INSERT INTO member_tasks (
        ...
        'automation_trigger', COALESCE(p_created_by, auth.uid())
...
BEGIN
    ...
    SELECT 
        COUNT(*) FILTER (WHERE assigned_at::date BETWEEN p_period_start AND p_period_end),
        COUNT(*) FILTER (WHERE last_contact_date::date BETWEEN p_period_start AND p_period_end),
        COUNT(*) FILTER (WHERE converted_at::date BETWEEN p_period_start AND p_period_end)
    INTO v_leads_assigned, v_leads_contacted, v_leads_converted
    FROM leads
    WHERE tenant_id = p_tenant_id AND assigned_to = p_member_id;
...
-- WhatsApp conversation update trigger
...
INSERT INTO migrations_log (version, description, executed_at) 
VALUES ('4B.1.0', 'FASE 4B: Member Tools & Integrations - Migration completed successfully', NOW());

-- Migration summary
DO $$
BEGIN
    RAISE NOTICE '=====================================================';
    RAISE NOTICE 'FASE 4B - MEMBER TOOLS & INTEGRATIONS';
