...
-- 3. VERIFICAR E CORRIGIR RELACIONAMENTO pipelines -> pipeline_stages
DO $$
BEGIN
...
-- 4. CRIAR TABELAS DE CADÊNCIA (principais)
...
-- 5. CRIAR TABELA lead_tasks (sem depender da tabela leads)
...
-- 7. CRIAR FOREIGN KEYS (apenas se as tabelas referenciadas existirem)
DO $$
BEGIN
    ...
    -- FK cadence_executions -> pipelines (se existir)
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'pipelines') THEN
        IF NOT EXISTS (
            SELECT 1 FROM information_schema.table_constraints 
    ...
    -- FK lead_tasks -> pipelines (se existir)
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'pipelines') THEN
        IF NOT EXISTS (
            SELECT 1 FROM information_schema.table_constraints 
    ...
    -- FK pipeline_win_loss_reasons -> pipelines (se existir)
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'pipelines') THEN
        IF NOT EXISTS (
            SELECT 1 FROM information_schema.table_constraints 
...
-- 8. ADICIONAR tenant_id nas tabelas que precisam (somente se as tabelas existirem)
DO $$
BEGIN
    ...
    -- Adicionar tenant_id na tabela leads se existir
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'leads') THEN
        IF NOT EXISTS (
            ...
            -- Pegar tenant_id da pipeline relacionada se possível
...
-- 9. HABILITAR RLS para as tabelas criadas
...
-- 10. CRIAR POLÍTICAS RLS (somente se tenant_id existir)
DO $$
BEGIN
    -- Políticas para pipeline_win_loss_reasons
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'pipeline_win_loss_reasons' AND column_name = 'tenant_id') THEN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'cadence_config' AND column_name = 'tenant_id') THEN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'cadence_tasks' AND column_name = 'tenant_id') THEN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'cadence_executions' AND column_name = 'tenant_id') THEN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'lead_tasks' AND column_name = 'tenant_id') THEN
...
-- 11. INSERIR DADOS DE EXEMPLO (somente se pipelines existir)
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'pipelines') THEN
...
ORDER BY 
    CASE table_name
        ...
        WHEN 'pipeline_stages' THEN 2
        WHEN 'pipeline_win_loss_reasons' THEN 3
        WHEN 'cadence_config' THEN 4
        WHEN 'cadence_tasks' THEN 5
        WHEN 'cadence_executions' THEN 6
        WHEN 'lead_tasks' THEN 7
    END; 