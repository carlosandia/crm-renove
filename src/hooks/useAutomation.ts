...
export const useAutomation = (): UseAutomationReturn => {
  ...
  const loadRules = useCallback(async () => {
    try {
      ...
      const response = await fetch('import.meta.env.VITE_API_URL || 'http://127.0.0.1:3001'/api/automation/rules', {
  ...
  const loadMetrics = useCallback(async () => {
    try {
      ...
      const response = await fetch('import.meta.env.VITE_API_URL || 'http://127.0.0.1:3001'/api/automation/metrics', {
        headers: getAuthHeaders()
      });
  ...
  const loadEventDefinitions = useCallback(async () => {
    try {
      ...
      const response = await fetch('import.meta.env.VITE_API_URL || 'http://127.0.0.1:3001'/api/automation/events/definitions', {
        headers: getAuthHeaders()
      });
  ...
  const loadEventLog = useCallback(async (filters?: any) => {
    try {
      ...
      const response = await fetch(`import.meta.env.VITE_API_URL || 'http://127.0.0.1:3001'/api/automation/events/log?${queryParams}`, {
        headers: getAuthHeaders()
      });
  ...
  const updateRule = useCallback(async (ruleId: string, updates: Partial<BusinessRule>): Promise<BusinessRule> => {
    try {
      const response = await fetch(`import.meta.env.VITE_API_URL || 'http://127.0.0.1:3001'/api/automation/rules/${ruleId}`, {
        method: 'PUT',
        headers: getAuthHeaders(),
  ...
  const testRule = useCallback(async (ruleId: string, testData: Record<string, any>): Promise<TestRuleResult> => {
    try {
      const response = await fetch(`import.meta.env.VITE_API_URL || 'http://127.0.0.1:3001'/api/automation/rules/${ruleId}/test`, {
        method: 'POST',
        headers: getAuthHeaders(),
  ...
  const emitEvent = useCallback(async (
    ...
    try {
      const response = await fetch('import.meta.env.VITE_API_URL || 'http://127.0.0.1:3001'/api/automation/events', {
        method: 'POST',
        headers: getAuthHeaders(),
        body: JSON.stringify({
          type,
