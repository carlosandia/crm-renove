...
export const useCompanies = () => {
  ...
  /**
   * 🔧 CORREÇÃO CRÍTICA: Busca de empresas com fallback robusto
   */
  const fetchCompanies = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);
      console.log('🔍 [useCompanies] Carregando empresas...', { user: user?.email, role: user?.role });

      let companiesData: any[] = [];

      // 🔧 TENTATIVA 1: Backend API com autenticação (preferencial)
      if (authenticatedFetch && user) {
        console.log('🚀 [useCompanies] Tentando Backend API autenticada...');
        try {
          ...
          if (response.ok) {
            ...
            if (result.success && result.data) {
              ...
              console.log(`✅ [useCompanies] ${companiesData.length} empresas recebidas via Backend API`);
            } else {
          ...
          throw backendError; // Propagar erro para tentar fallback
        }
      } else {
        throw new Error('Sem contexto de autenticação disponível');
      }

    } catch (primaryError: any) {
      console.warn('⚠️ [useCompanies] Tentativa principal falhou, usando fallback Supabase:', primaryError.message);
      ...
      try {
        ...
        console.log(`🔄 [useCompanies] Fallback Supabase: ${companiesData.length} empresas encontradas`);
      } catch (supabaseError: any) {
        console.error('❌ [useCompanies] Fallback Supabase também falhou:', supabaseError.message);
        throw new Error(`Todas as tentativas falharam: ${supabaseError.message}`);
      }
    }
