...
// ✅ CORREÇÃO TEMPORÁRIA: Asserção de tipo para garantir que logger tem método info
const typedLogger = logger as typeof logger & { info: (message: string, context?: any) => void };
...
const ModernAdminPipelineManagerRefactored: React.FC<ModernAdminPipelineManagerRefactoredProps> = ({ 
  ...
  useEffect(() => {
    if (shouldLoadLeads) {
      logger.debug('Carregando leads para pipeline', {
        domain: 'pipeline-admin',
        pipelineName: selectedPipeline?.name,
        pipelineId: selectedPipeline?.id?.substring(0, 8)
      });
      refreshLeads();
    }
  ...
  const adminPipelines = useMemo(() => {
    ...
    if (shouldLog) {
      logger.debug('Recalculando adminPipelines', {
        domain: 'pipeline-admin',
        loading,
    ...
    if (loading) {
      if (shouldLog) {
        logger.debug('Aguardando carregamento de pipelines', { domain: 'pipeline-admin' });
      }
      return [];
    }
    ...
    if (!pipelines) {
      if (shouldLog) {
        logger.debug('Pipelines ainda não carregadas', { domain: 'pipeline-admin' });
      }
      return [];
    }
    ...
    if (!user?.role || (user.role !== 'admin' && user.role !== 'super_admin')) {
      if (shouldLog) {
        logger.warn('Usuário não é admin', { domain: 'pipeline-admin', userRole: user?.role });
      }
      return [];
    }
  ...
  const filteredPipelines = useMemo(() => {
    ...
    logger.debug('Filtragem aplicada', {
      domain: 'pipeline-admin',
  ...
  const handleCreatePipeline = useCallback(() => {
    typedLogger.info('Criando nova pipeline', { domain: 'pipeline-admin', action: 'create' });
    setEditingPipeline(null);
    setViewMode('create');
  }, []);

  const handleEditPipeline = useCallback((pipeline: Pipeline) => {
    typedLogger.info('Editando pipeline', { 
      domain: 'pipeline-admin', 
      action: 'edit',
      ...
      case 'view':
        ...
        return (
          <PipelineKanbanView
            ...
            autoRefresh={true}
            onViewDetails={(lead) => {
              console.log('🎯 [ModernAdminPipelineManagerRefactored] onViewDetails callback recebido:', {
                leadId: lead.id.substring(0, 8),
                leadName: (lead.first_name || '') + ' ' + (lead.last_name || ''),
                hasModalManager: !!modalManager,
                hasOpenDealDetailsModal: !!modalManager.openDealDetailsModal
              });
              modalManager.openDealDetailsModal(lead);
            }}
          />
        );
    ...
    <div className={`h-full flex flex-col ${className}`}>
      ...
      {modalManager.isDealDetailsModalOpen && modalManager.modalData && (
        <PipelineLeadDetailsModal
          ...
          customFields={viewingPipeline?.pipeline_custom_fields || []}
          pipelineId={viewingPipeline?.id}
          onUpdate={async (leadId: string, updatedData: any) => {
